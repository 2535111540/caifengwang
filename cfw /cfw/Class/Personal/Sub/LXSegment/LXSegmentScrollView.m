...
-(instancetype)initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _segmentToolView=[[LiuXSegmentView alloc] initWithFrame:CGRectMake(0, 0, MainScreen_W, 35) titles:titleArray clickBlick:^void(NSInteger index) {
            NSLog(@"-----%ld",index);
            [_bgScrollView setContentOffset:CGPointMake(MainScreen_W*(index), 0)];
        }];
...
-(UIScrollView *)bgScrollView{
    if (!_bgScrollView) {
        ...
        _bgScrollView.contentSize=CGSizeMake(MainScreen_W*2, self.bounds.size.height-_segmentToolView.bounds.size.height);
