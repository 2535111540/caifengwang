...
{
    self = [super initWithFrame:CGRectMake(kScreenWidth - 150, 0, kScreenWidth, kScreenHeight)];
    if (self) {
        if (rowHeight <= 0) {
            rowHeight = 44;
        }
...
//    [colors[serie] setFill];
    ...
    CGContextMoveToPoint(context,
                         LeftToView * 12, TopToView * 0.5);//设置起点
    
    CGContextAddLineToPoint(context,
                            LeftToView * 11.5, TopToView);
    
    CGContextAddLineToPoint(context,
                            LeftToView * 12.5, TopToView);
